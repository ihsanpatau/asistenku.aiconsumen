/* notif.js â€” sistem notifikasi nyata untuk asistenku.pro. Notifikasi dibangun dari aktivitas & kondisi akun yang sesungguhnya: dokumen yang baru selesai dibuat (RiwayatStore), kuota yang hampir habis, masa paket yang akan berakhir, dan pesan selamat datang untuk akun baru. Tidak ada data contoh/template â€” kalau tidak ada aktivitas, panel akan menampilkan status kosong yang jujur. */
const AkNotif = (function () {
  ...
  function build() {
    ...
    // 2) Peringatan kuota hampir habis (â‰¥80% terpakai)
    if (window.AkAccount) {
    ...
    // 3) Masa paket akan berakhir (â‰¤7 hari)
    ...
    if (exp) {
      ...
      if (days >= 0 && days <= 7) {
        list.push({
          ...
          desc:
            days === 0
              ? "Paket Anda berakhir hari ini"
              : `Berakhir dalam ${days} hari lagi \u2013 perpanjang agar tidak terganggu`,
          waktu: new Date().toISOString(),
          link: "upgrade.html",
        });
      }
  ...
  async function updateBadges() {
    // PERBAIKAN BUG: kalau unreadCount() gagal (mis. Supabase belum siap /
    // koneksi bermasalah), jangan biarkan badge menggantung di kondisi lama â€”
    // sembunyikan dulu (aman & jujur: tidak ada info dianggap tidak ada
    // notifikasi baru) daripada memunculkan titik notifikasi yang salah.
    let n = 0;
    try {
      n = await unreadCount();
    } catch (e) {
  ...
  async function render() {
    ...
    if (list.length === 0) {
      body.innerHTML =
        ...
        '<div style="font-size:38px;margin-bottom:10px;">ðŸ””</div>' +
    ...
    body.innerHTML = list
      .map(function (n) {
        ...
        return (
          ...
          (isRead ? "" : " unread") +
          '" onclick="AkNotif.open(\'' +
          n.id +
          "','" +
          (n.link || "") +
          "')\">" +
          '<div class="ak-notif-icon ' +
          n.icon +
          '">' +
          (ICONS[n.icon] || ICONS.info) +
          "</div>" +
          '<div class="ak-notif-body">' +
          '<div class="ak-notif-title">' +
          n.title +
          "</div>" +
          '<div class="ak-notif-desc">' +
          n.desc +
          "</div>" +
          '<div class="ak-notif-time">' +
          relatif(n.waktu) +
          "</div>" +
          "</div>" +
  ...
  async function openPanel() {
    ...
    if (body)
      body.innerHTML =
        '<div style="text-align:center;padding:30px 10px;color:var(--gray-400);font-size:13px;">Memuat...</div>';
    await render();
  }
  function closePanel() {
  ...
  async function markAllRead() {
    const all = (await buildAll()).map(function (n) {
      return n.id;
    });
    const read = getIds(READ_KEY);
